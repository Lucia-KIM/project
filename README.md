```
이곳은 Data Science를 지향하며, 한 걸음씩 나아가고 있는
데이터 분석가 김아람의 프로젝트 모음 공간입니다.
각 프로젝트의 최종결과물은 개별 링크를 통해 확인하실 수 있습니다. 
```

# [데이터분석 프로젝트]

## 1. 패션 상품 구매 데이터 분석을 통한 브랜드 및 고객 분석 (기업 실데이터 사용)
...
> [분석 최종 코드](https://nbviewer.jupyter.org/github/Lucia-KIM/project/blob/53dfbf04d41503f83326d21ec2c0b6fee0adedfd/%EA%B3%B5%EC%9C%A0%EC%A3%BC%EC%B0%A8%EC%9E%A5%20%EA%B8%B0%EC%97%85%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EB%B6%84%EC%84%9D/Final_code_Marketing_Modified.ipynb) / [분석 결과 Insight & Action Plan](https://github.com/Lucia-KIM/project/blob/main/%EA%B3%B5%EC%9C%A0%EC%A3%BC%EC%B0%A8%EC%9E%A5%20%EA%B8%B0%EC%97%85%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EB%B6%84%EC%84%9D/Data%20Analysis%20Insight_Marketing_1.pdf)
...
---
## 3. 국민연금DB 이용 유니콘 기업 발굴하기 (기업 실데이터 사용)  
분석 목표 : 국민연금고지금액, 연매출액, 직원수를 포함한 데이터를 가지고 유니콘기업으로 보이는 기업들을 나름의 방식으로 찾아내는 것
...
- 년단위데이터: 매출액(천 원 단위)
> 유니콘 기업 정의 : 유니콘 기업(Unicorn)은 기업 가치가 10억 달러(=1조 원) 이상이고 창업한 지 10년 이하인 비상장 스타트업 기업을 말함 

#### 2)프로젝트 접근 방향
1. 구직사이트 CATCH에서 제공하는 국내 [매출액 TOP10 스타트업](https://m.catch.co.kr/Comp/CompThemeRank?TCode=163) 자료를 바탕으로 유니콘 기업에 대한 가설을 정의  
2. 데이터 탐색을 통해 변수간 상관관계 살피기 
3. 캐치 데이터를 바탕으로 세운 가설과 비슷한 성향을 보이는 기업 선택 
4. 실제 유니콘 기업의 정보를 크롤링으로 수집하여 비교
5. 유니콘 기업으로 예상되는 최종 5곳 선정
---
## 4. 주식가격 분석하기
...
1. 데이터 살펴보기
2. 데이터 분류
 - NaN 값 포함 여부에 따른 분류 : NaN 값이 포함된 데이터만 따로 추출하여 비교
 - 종목별 가격 구간에 따른 분류 : 종목(Symbol)에 따라 데이터 내 가장 최신일자(마지막 일자 : 2020-09-18 )를 기준으로 가격대(고/중/저)를 구분 
  > * 고가 : 100,000 이상인 종목 
  > * 중가 : 10,000 ~ 100,000 사이인 종목    
  > * 저가 : 10,000 미만인 종목
3. 가격대별 샘플 비교 : 
앞서 정의한 (고/중/저)가격대를 기준으로 각 그룹 샘플 10개 종목을 무작위 추출하여 비교
 - 각 샘플별 가격 변동 그래프
...
분석을 위한 데이터 전처리는 파이썬 언어를 사용한 Jupyter notebook환경에서 진행하였으며, 시각화는 Tableau를 사용하였음  
...
2. 유저 가입 정보: id, 가입일
3. 소비 행동 정보: 상품id, 점포 주소, 결제일, 결제건수
4. 외부 정보: 기온, 강수량, 풍속 등 기상데이터

#### 2)Feature Engineering 방향
1. 유저 데이터 구분
2. 기상데이터 활용
3. AD1 구분 및 STORE_ID 제거
4. Category 변수 One-Hot-Encoding

---

## 2. 미국 성인 인구조사 소득 예측 대회(캐글)
대회 목표 : 미국 성인 인구조사 데이터셋을 이용하여 소득이 50K를 넘을지 예측 [대회정보](https://www.kaggle.com/c/kakr-4th-competition/overview)

** _본 대회는 캐글코리아에서 주최한 데이터분석 캠프에 참여하여 학습한 내용으로 실제 대회는 아니라는 점을 말씀드립니다._

> [모델링 최종 코드 확인](https://gist.github.com/Lucia-KIM/b11709f11f15acce8a94eaa3294e1b42)  /
...
#### 1)데이터 정보
Train/Test 데이터는 다음의 columns으로 구성되어 있고, train은 예측해야 하는 target 값 컬럼 1개를 더 포함하고 있음 

각 데이터가 의미하는 바는 아래와 같음  
 - age : 나이
 - workclass : 고용 형태
...
1. fnlwgt : 변수간 상관관계 분석 및 Kmeans기법으로 군집화 시도
2. Race : 비슷한 그룹으로 묶어 바이너리로 정수 인코딩
3. capital gain/loss : 구간화, 상계처리(파생변수 생성)
4. age : 구간화(20세부터 5세 단위) 20세 이하, 65세 이상 카테고리화
